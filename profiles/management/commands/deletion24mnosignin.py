...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        profiles = Profile.objects.select_related('user').filter(Q(user__last_login__lt = timezone.now() - timedelta(weeks=104)) | Q(user__last_login = None))
        if profiles:
            ...
            for p in profiles:
                ...
                html_message = render_to_string('profiles/emails/deletion_24m_no_signin_email_inline.html', email_context)
                plain_message = render_to_string('profiles/emails/deletion_24m_no_signin_email_plain.html', email_context)
                try:
            ...
            self.stdout.write(self.style.SUCCESS('Successfully deleted profiles that did not sign in in the past 24 months and informed them by email. Emails sent to: ' + profileslist))
        else:
            self.stdout.write(self.style.SUCCESS('All the profiles have at least 1 connection in the past 24 months. No email sent.'))
